...
const Settings = props => {
  ...
  const [committeeReportsCount, setCommitteeReportsCount] = useState(0)
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await OfficerService.countOfficer()
        const countOfficer = data.count

        // Check if the component is still mounted before updating state
        if (isMounted) {
          setOfficerCount(countOfficer)
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await MemberService.countMember()
        const countMember = data.count
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        const data = await DownloadService.downloadCount();
        console.log(data)
        const actsCount = data.count[0]; 
        const committeeReportsCount = data.count[1];
        const totalDownloads = actsCount + committeeReportsCount; 

        // Check if the component is still mounted before updating state
        if (isMounted) {
          setTotalDownloads(totalDownloads);
          setActsCount(actsCount);
          setCommitteeReportsCount(committeeReportsCount);
        }
      } catch (error) {
    ...
    <React.Fragment>
      <Col xl={4}>
        ...
        <Card>
          <CardBody>
            <div className="d-flex flex-wrap align-items-start">
              <h5 className="card-title mb-3 me-2">Downloads </h5>
            ...
            <div className="d-flex flex-wrap">
              <div>
                <p className="text-muted mb-1">Total Downloads  <h4 className="mb-3  text-center">{totalDownloads} </h4></p>
                <p className="text-muted mb-1"> Acts : {actsCount} </p> <p> Committee Reports : {committeeReportsCount}</p>
              </div>
              <div className="ms-auto align-self-end">
                <i className="bx bx-download display-4 text-light"></i>
              </div>
            </div>
          </CardBody>
        </Card>
          ...
          <CardBody>
            ...
            <div className="d-flex flex-wrap">
              <div>
                ...
                <h4 className="mb-3">{officerCount}</h4>
              </div>
              <div className="ms-auto align-self-end">
          ...
          <CardBody>
            ...
            <div className="d-flex flex-wrap">
              <div>
                ...
                <h4 className="mb-3">{memberCount}</h4>
              </div>
              <div className="ms-auto align-self-end">
                <i className="bx bx-group display-4 text-light"></i>
              </div>
